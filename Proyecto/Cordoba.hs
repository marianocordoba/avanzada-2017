...
encrypt [] _ = []
encrypt xs ks 
  | length ks /= keySize = error "Clave inválida"
  | otherwise = bitsToText (join (encryptChunks (split (textToBits xs) keySize) ks))
...
decrypt xs ks 
  | length ks /= keySize = error "Clave inválida"
  | otherwise = bitsToText (join (decryptChunks (split (textToBits xs) keySize) ks))
